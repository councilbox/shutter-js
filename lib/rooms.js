...
const STOP_SOUP_RECORDING = `
	mutation stopSoupRecording($roomNumber: String!) {
		stopSoupRecording(roomNumber: $roomNumber) {
			roomNumber
			recording
		}
	}
...
module.exports = class Rooms {
	...
	async stopSoupRecording({ roomNumber }) {
		const variables = { roomNumber };

		const response = await this.#shutterFetch({
			query: STOP_SOUP_RECORDING,
			variables,
			operationName: getOperationName(STOP_SOUP_RECORDING)
		});

		if (response.errors) throw response.errors;

		return response.data.stopSoupRecording;
	}
